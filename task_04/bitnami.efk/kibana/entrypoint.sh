...
while true; do
    token_value=$(curl -k -s -X POST -u elastic:elastic "https://elasticsearch:9200/_security/service/elastic/kibana/credential/token/kibana_token?pretty" | grep value | cut -d ':' -f 2)

    if ! curl -k -s https://elasticsearch:9200 >/dev/null; then
    ...
    if [ -n "$token_value" ]; then
        ...
        curl -k -s -X DELETE -u elastic:elastic "https://elasticsearch:9200/_security/service/elastic/kibana/credential/token/kibana_token" > /dev/null
        token_value=$(curl -k -s -X POST -u elastic:elastic "https://elasticsearch:9200/_security/service/elastic/kibana/credential/token/kibana_token?pretty" | grep value | cut -d ':' -f 2)
        sed -i "s|^\(elasticsearch.serviceAccountToken:\).*|\1 ${token_value}|g" ${KIBANA_CONFIG}
        # echo "Token retrieved: $token_value"
        # cat ${KIBANA_CONFIG}
