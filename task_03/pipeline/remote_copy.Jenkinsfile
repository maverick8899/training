            ...
            steps {
                script {
                    ...
                    sshagent(credentials: ['vagrant-key']) {
                        try {    
                            if (userChoice == 'linux') {
                                sh "/home/jenkins/remote_copy.sh ${params.SOURCE_FILE} ${params.INVENTORY}"
                                archiveArtifacts artifacts: "remote_copy_${BUILD_NUMBER}.log", allowEmptyArchive: true
                                sh "rm -f ${WORKSPACE}/remote_copy_${BUILD_NUMBER}.log"
                            } else if (userChoice == 'python') {
                                sh 'echo "Running Python script..."'
                                sh "python3 /home/jenkins/remote_copy.py ${params.INVENTORY} ${params.COMMAND}"
                                archiveArtifacts artifacts: "remote_copy_${BUILD_NUMBER}.log", allowEmptyArchive: true
                                sh "rm -f ${WORKSPACE}/remote_copy_${BUILD_NUMBER}.log"
